...
class Model():
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'idt_A', 'D_B', 'G_B', 'cycle_B', 'idt_B', 'tv_G_A', 'tv_G_B',
                           'perception_G_A', 'perception_G_B', 'image_G_A', 'image_G_B', 'rl_G_A', 'rl_G_B']
        ...
        if self.isTrain:
            ...
            unet = torch.hub.load('mateuszbuda/brain-segmentation-pytorch', 'unet',
                                   in_channels=3, out_channels=1, init_features=32, pretrained=True).eval()
            loss_network = nn.Sequential(*list((unet.encoder1, unet.pool1,
                                               unet.encoder2, unet.pool2,
                                               unet.encoder3, unet.pool3,
                                               unet.encoder4, unet.pool4))).eval()
    ...
    def backward_G(self):
        ...
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A + \
                      self.loss_idt_B + self.loss_tv_G_A + self.loss_tv_G_B + self.loss_rl_G_A + self.loss_rl_G_B + \
    ...
    def optimize_parameters(self):
        ...
        return self.real_A, self.fake_A, self.real_B, self.fake_B,\
               self.loss_G_A, self.loss_G_B, self.loss_D_A, self.loss_D_B
